...
window.dhtmlXScheduler = window.scheduler = {
	...
	if(!t) {e = this._pre_render_events(e, t); }
	else{
	var startDate_1=scheduler.getState().date;
	var dateFrom = new Date(startDate_1.getFullYear(),startDate_1.getMonth(),startDate_1.getDate(),00,00,00); 
	var dateTo = new Date(startDate_1.getFullYear(),startDate_1.getMonth(),startDate_1.getDate(),23,59,59); 

	e = scheduler.getEvents(dateFrom,dateTo); 
	e.sort(function(a,b){ return (a.start_date > b.start_date ? 1 : -1); });
	}
	//alert(e.toSource());
	var count=0;
        for (var i = 0; i < e.length; i++)
            ...
            if (this._table_view)
                ...
                } else { if(e[i].text == "New event") continue; 
			this.render_event(e[i],count);count=count+20;
	} //==============Shusmita 13-01-2016
        ...
        this._rendered = []
    }, scheduler.updateEvent = function(e,top) {  //alert("Update"+e);
	...
	//alert(t.start_date);
    ...
    }, scheduler.render_event = function(e,top) { //alert("Rendering gggg "+e.toSource());
		...
		//s.clientWidth=1800000;
		    ...
		    l = (s.clientWidth*so)/1800,
                ...
                var g = this._render_v_bar(e, i + h, n, l, r, e._text_style, scheduler.templates.event_header(e.start_date, e.end_date, e), scheduler.templates.event_text(e.start_date, e.end_date, e),'',top);
    ...
    }, scheduler._render_v_bar = function(e, t, i, s, a, n, r, d, o,top) {// alert('left' + t); alert('top' + i);
        //alert("Rendering ");
	var minute2 = (e.start_date).getMinutes();
        var second2 = (e.start_date).getSeconds();
        var pix = (minute2*60)+second2;
	var sp = scheduler.locate_holder(e._sday); //alert(top);
	...
	topPix=topPix+top; //(countEvent*heightPix); //alert(topPix);
	   ...
	   // alert(s.toSource());
	    ...
	    if(i.text == 'New event'){
		   ...
		   // }
		     ...
		     document.getElementById("pgmName").value=i.pgmName;
		     document.getElementById("pgm_id").value=i.pgm_id;
			...
			var sendDate=startDate+";"+i.rate_agreement; //alert(sendDate);
	...
	var newEventID=(this.getEvent(this._lightbox_id)).id; //alert(e.toSource());
	...
	if(e.pgmName == "" ){			
		alert ("No Program Found in scheduler for this start time.");
		return;
	}
	if(e.text == 'COM' && e.rate_agreement == "")
	{
	...
	//if(e.serial_type == "" ){
		//alert("Commercial type is mandatory");
		//return;
	//}
	//if(e.bpcode == "" ){
		//alert("Business Partner Code is mandatory");
		//return;
	//}
	//if(e.order_ref == "" ){
	...
	if((e.input_type == "1") || (e.input_type == "0") || (e.input_type == "")){ 
			...
			"UpdateEvents.php",     
		{
			...
			{  
				...
				isHide=false;
				alert("Could not save. "+obj);
			     }
			     else{
 				 var splitObj=obj.split("|"); 
				 var rateAgreement_LineID=splitObj[0];
				 var pgm_episodeno=splitObj[1];
				 eObj.rateAgreement_Line=rateAgreement_LineID;
				 eObj.episode=pgm_episodeno;
				 isHide=true;
				if(isHide == true){
