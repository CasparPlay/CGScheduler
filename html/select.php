...
<head>
	...
	<style type="text/css" media="screen">
		...
		.dhx_cal_event.event_LIVE_1 div, .dhx_cal_event_line.event_LIVE_1{
			background-color: #FF0000 !important;
			border-color: #839595 !important;
		}
		.dhx_cal_event_clear.event_LIVE_1{
			color:#B82594 !important;
		}
		.dhx_cal_event.event_LIVE_2 div, .dhx_cal_event_line.event_LIVE_2{
			background-color: #FF0000 !important;
			border-color: #839595 !important;
		}
		.dhx_cal_event_clear.event_LIVE_2{
			color:#B82594 !important;
		}
		.dhx_cal_event.event_LIVE_3 div, .dhx_cal_event_line.event_LIVE_3{
			background-color: #FF0000 !important;
			border-color: #839595 !important;
		}
		.dhx_cal_event_clear.event_LIVE_3{
			color:#B82594 !important;
		}
		.dhx_cal_event.event_LIVE_4 div, .dhx_cal_event_line.event_LIVE_4{
			background-color: #FF0000 !important;
			border-color: #839595 !important;
		}
		.dhx_cal_event_clear.event_LIVE_4{
			color:#B82594 !important;
		}

		.dhx_cal_event.event_COM div, .dhx_cal_event_line.event_COM{
			background-color: #FC5BD5 !important;
			border-color: #839595 !important;
		}
		.dhx_cal_event_clear.event_COM{
			color:#B82594 !important;
		}
		.dhx_cal_event {
			z-index: 1;
...
function checkComValidation(){
		...
		function serverRef() { 
			 ...
			 if((scheduler.formSection('Server Reference').getValue()) == 'COM'){
			   document.getElementById('rate_agreement').disabled = false; 
			}
			else{
  			 	document.getElementById('rate_agreement').disabled = true;
	...
	function getOrderRefList(){ 
		...
		function getAssetID(){  scheduler.formSection('Asset ID').setValue('');
		...
		function showScheduleStatus(){ 
			var hour = document.getElementById("scheduleStatus").value; //alert(razuna_name);
			var date1 = scheduler.getState().date; 
			var day=date1.getDate();
			var month=date1.getMonth()+1;
			var hour2 = parseInt(hour)+1;
			if(date1.getDate() < 10) { day='0'+date1.getDate();}
			if(month < 10) { month='0'+month;} 
			if(parseInt(hour) < 10) { hour='0'+hour;} 
			if(parseInt(hour2) < 10) { hour2='0'+hour2;} 
			var startDate= date1.getFullYear()+ '-' +month+ '-' + day+" "+hour+":"+"00"+"00";
			var endDate= date1.getFullYear()+ '-' +month+ '-' + day+" "+hour2+":"+"00"+"00";
			if(parseInt(hour2) > 23) {endDate= date1.getFullYear()+ '-' +month+ '-' + (parseInt(day)+1)+" "+"00"+":"+"00"+"00";} 
			var sendDate=startDate+";"+endDate;

		 $.post(                            
		        "scheduleStatusPerHour.php",     
		        {
		            field: "name",
		            name: sendDate
		        }                               
			    ...
			    document.getElementById('scstatus').innerHTML=obj;
		...
		function durationhour(event){
			...
			var duration1 = endhour + ":" + endmin+ ":" + endSec+":"+totalFrame;  
...
var p="<div class='dhx_cal_ltext' style='height:50px;border:1px solid red;color:black;'>Program Name :<div style='border:0px solid blue;margin-left:70px;width:50px;height:30px;margin-top:-10px' class='dhx_cal_navline'></div><input style='border:1px solid blue;margin-top:10px;margin-left:10px;height:30px;width:80%' type='text' id='prog' name='prog' onchange='getAssetID()' >&nbsp; <input style='border:1px solid blue;margin-top:10px;margin-left:10px;height:30px;width:50px' type='text' id='episode' name='episode' onchange='getAssetID()' hidden >&nbsp; <input style='border:1px solid blue;margin-top:10px;margin-left:10px;height:30px;width:50px' type='text' id='segment' name='segment' onchange='getAssetID()' hidden></div>";
			...
			get_value:function(node,ev){
				...
				ev.episode=document.getElementById("episode").value;
			...
			scheduler.config.lightbox.sections = [
				{ name: "Server Reference", height: 30, map_to: "text", type: "select", focus: true, options:[
						{key:"COM", label:"COM"}
				] , filtering : true},
				...
				{ name:"Order Reference", height:25, type:"textarea", map_to:"order_ref",focus: true},
				{ name:"BP Code", height:20, type:"textarea", map_to:"bp_code",focus: true },
			...
			scheduler.config.lightbox.sections = [
				{ name: "Server Reference", height: 30, map_to: "text", type: "select", focus: true, options:[
						{key:"PGM", label:"PGM"},
						{key:"LIVE_1", label:"LIVE_1"},
						{key:"LIVE_2", label:"LIVE_2"},
						{key:"LIVE_3", label:"LIVE_3"},
						{key:"LIVE_4", label:"LIVE_4"},
				] , filtering : true},
				{ name: "Serial Type", height: 30, map_to: "serial_type", type: "select", focus: true, options:[
						{key:"MEGA", label:"MEGA"},
						{key:"DRAMA", label:"DRAMA"},
				] , filtering : true},
				//{ name: "Program name", height: 25, type: "textarea", map_to: "program", focus: true},
				{ name:"Program Name ", height:100, map_to:"prog", type:"program_info" , focus:true},
				{ name: "Asset ID", height: 25, type: "textarea", map_to: "asset",focus: true},
                                { name: "Formats", height: 30, type: "select", map_to: "format", focus: true, options:[
						{key:"Now", label:"Now"},
			...
			else if(role == "SCHEDULER" || role == "MCR"){
			scheduler.config.lightbox.sections = [
				{ name: "Server Reference", height: 30, map_to: "text", type: "select", focus: true, options:[
						{key:"PGM", label:"PGM"},
						{key:"PROMO", label:"PROMO"},
				...
				{ name:"Program Info ", height:100, map_to:"prog", type:"program_info" , focus:true},
				{ name: "Asset ID", height: 25, type: "textarea", map_to: "asset",focus: true},
                               /* { name: "Formats", height: 30, type: "select", map_to: "format", focus: true, options:[
						{key:"Now", label:"Now"},
			...
			scheduler.config.hour_size_px=(60/step)*3000;
			var id=0;
			...
			for (var i=0; i<60/step; i++){
				...
				html+="<div id='"+'slot'+id+"' style='height:100px;line-height:15px;border-bottom:0px solid #CECECE;color:black;font-size:14px'></div>";
			...
			scheduler.attachEvent("onLightbox", function(){ 
			   ...
			   //=================Asset ID ==============
			    scheduler.formSection("Server Reference").control.onchange=function(){serverRef();};
...
<div style="height:30px;width:98%;border:0px solid red;float:middle;margin-left:0%;margin-top:2px;display:block">
 <span  style='color:blue;font-size:24px,font-weight:bold'><b>Commercial Schedule Status Hour</b></span>&nbsp;&nbsp;
 <select name='scheduleStatus' id='scheduleStatus' onchange='showScheduleStatus()' >
<?php $i=0; while($i<24 ) {echo '<option value='.$i.'>'.$i.'-'.($i+1). '</option>';  $i++;} ?>
</select>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<b><span id='scstatus' style='color:#B82594;font-size:36px,font-weight:bold'></span></b>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
<input name="check" type="button" id="check" value="Validate Rate Agreement" onclick="checkComValidation();" style="height:30px;width:200px;color:black;font-weight:bold;float:right;"/>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
Validate Hour:
