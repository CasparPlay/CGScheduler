<?php
	...
	//=========================== Tauwab ====================================================================
        $sql = "SELECT VID_ID,FOLDER_ID_R,VID_NAME_ORG,VID_META FROM `raz1_videos` WHERE VID_FILENAME = '$pname' ";
        //=======================================================================================================
			...
			$sendData=$asset_id.'||'.$content;	
                ...
                */
                //=============================tauwab=====================
                 ...
                 $message = "";
                 $message = exec("ffmpeg -i ".$FinalLink." 2>&1 | grep inaccurate");
                 if ($message == "") 
                 {
                   $sendData=$asset_id.'||'.$content;
                 }
                else 
                 {
                   ...
                   if(sizeof($arr) > 0){
                    ...
                    if(sizeof($arr) > 2)
                    {
                    $content = $arr[1].":".$arr[2].":".$arr[3];
                    }
                    else
                    {
                     $content = explode(" ",trim($arr[1])); // split Duration : 10.28 s and split 10.28 s again
                     $content = "0:0:".$content[0]; //get only 10.28
                    }
			        $sendData=$asset_id.'||'.$content;	
                    }
		         }
