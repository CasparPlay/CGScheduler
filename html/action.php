<?php 
    ...
    //======================================================================
    $CreatePlaylistTime = $date2.Date(' H:i:s');
    $delete_date = Date('Y-m-d H:i:s',strtotime("-2 days",strtotime($CreatePlaylistTime)));
    //======================================================================
 	//Establishing Connection with locaslhost Server by passing server_name, user_id and password as a parameter 

	$connection = mysql_connect("172.16.10.88", "root", "password") or die('Could not connect to localhost. ' . mysql_error());
	...
	{ 
      ...
      //================================================================================================
		...
		while ($row = mysql_fetch_assoc($result)) 
		{
			...
			//================== Primary Server ===========================
			$check_sql = "SELECT scheduler_id,state FROM `playlist_1` WHERE scheduler_id = '$id'" ;
			...
			if($scheduler_id > 0 ){ 
				if($state != 1 && $state != 2){
					$sql1 = "Update playlist_1 set label='$text', name='$razuna_name',timecode='$timecode', starttime='$start_date',state='0',updated='$date3',updatedby='$login_session',asset_id='$asset_id' where scheduler_id='$id'";  
				}
			}
			else{ 
				$sql1 = "INSERT INTO playlist_1 (label, name, timecode, starttime, seek, length, state, pushtime, asset_id,scheduler_id,created,createdby) VALUES ('$text', '$razuna_name', '$timecode', '$start_date', '0', '0', '0', '$start_date', '$asset_id','$id','$date3','$login_session')"; 
			}
			if($sql1 != ""){ 
			...
			$check_sql1 = "SELECT scheduler_id,state FROM `playlist_1` WHERE scheduler_id = '$id'" ;
			...
			if($scheduler_id1 > 0 ){ 
				if($state1 != 1 && $state1 != 2){
					$sql2 = "Update playlist_1 set label='$text', name='$razuna_name',timecode='$timecode', starttime='$start_date',state='0',updated='$date3',updatedby='$login_session',asset_id='$asset_id' where scheduler_id='$id'"; 
				}
			}
			else{
				$sql2 = "INSERT INTO playlist_1 (label, name, timecode, starttime, seek, length, state, pushtime, asset_id,scheduler_id,created,createdby) VALUES ('$text', '$razuna_name', '$timecode', '$start_date', '0', '0', '0', '$start_date', '$asset_id','$id','$date3','$login_session')"; 
			}
